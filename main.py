...
def start(bot, update):
    ...
    Para finalizar, escriba /end''',parse_mode='HTML')
...
def book_res(bot,update,user_data):
    ...
    if user_data['table']=='Cualquiera':
        try:
            ...
            conn.commit()
            c.execute('''SELECT * FROM Tables NATURAL JOIN (SELECT Table FROM Tables EXCEPT SELECT Table FROM Reservations WHERE Date='%s') ORDER BY Price ASC ''' % (user_data['date']))
            result=c.fetchall()
            c.close()
            conn.close()
            ...
            for row in result:
                if row[2]>max:
                    ...
                    max=row[2]
                update.message.reply_text(emojize(":large_blue_circle:",use_aliases=True)+'Table #'+str(row[0])+' | '+str(row[1])+' | Places: '+str(row[2])+' | Price: ' + str(row[3]) + ' RUB')
        ...
        try:
            ...
            c.execute('PRAGMA foreign_keys=ON;')
            conn.commit()
            c.execute('''SELECT * FROM Tables NATURAL JOIN (SELECT Table FROM Tables EXCEPT SELECT Table FROM Reservations WHERE Date='%s') WHERE Type = '%s' ORDER BY Price ASC ''' % (user_data['date'], str(user_data['table'])))
            result = c.fetchall()
            c.close()
            conn.close()
            ...
            for row in result:
                if row[2] > max:
                    user_data['biggest']=row[0]
                    max=row[2]
                update.message.reply_text(emojize(":large_blue_circle:", use_aliases=True)+'Mesa N°' + str(row[0]) + ' | Ubicacion: ' + str(row[2]) + ' | Precio: S/' + str(row[3]))
            reply_keyboard = [['Economico','Grande']]
            user_data['cheapest']=result[0][0]
            update.message.reply_text('Ingrese el numero de mesa o elija una de las opciones sugeridas.', reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
        except:
...
def menu_show(bot, update, user_data):
    ...
    if user_data['type'] == 'Todas':
        ...
        result = c.fetchall()
        for row in result:
            update.message.reply_text(emojize(":fork_and_knife:", use_aliases=True)+'Platillo: ' + str(row[1]) + ' | Tipo: ' + str(row[2]) + '\n Ingredientes: ' + str(row[3]) + '\n Precio:S/ ' + str(row[4]))
    else:
        if update.message.text == 'Si':
        ...
        if user_data['type'] == 'Tradicional':
            ...
            s = ':spaghetti:'
        for row in result:
            update.message.reply_text(emojize(s, use_aliases=True)+'Platillo: ' + str(row[1]) + '\n Ingredientes: ' + str(row[3]) + '\n Precio:S/ ' + str(row[4]))
    reply_keyboard = [[str(row[1])] for row in result]
    update.message.reply_text('''Por favor seleccione el plato a ordenar.\n Tambien puede escribirlo.''', reply_markup= ReplyKeyboardMarkup(reply_keyboard,one_time_keyboard=True))
    c.close()
    conn.close()
...
def end(bot,update,user_data):
    update.message.reply_text('Hasta luego, ¡que lo disfrute!\n Para ayuda, escriba /start',reply_markup= ReplyKeyboardRemove( ))
    user_data.clear()
    return ConversationHandler.END
